    ...
    public class CopyGroup : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            try
            {
                ...
                XYZ centerUserRoom = GetElementCenter(userRoom);
                XYZ userPoint = centerUserRoom+offset;
                Transaction transaction = new Transaction(doc);
                transaction.Start("Коприрование группы объектов");
                doc.Create.PlaceGroup(userPoint, group.GroupType);
                transaction.Commit();
            }
